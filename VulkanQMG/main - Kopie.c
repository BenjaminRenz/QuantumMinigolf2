...
struct CameraListItem* getCameras(unsigned int* numberOfCameras){ //TODO change to return char list with names
    ...
    while(S_OK==myCameralist->lpVtbl->Next(myCameralist,1,&myCamera,NULL)){
        ...
        VARIANT VariantField; //Do not set to =0 or we will get access violation TODO do with malloc?
...
void uninitialize_all(struct CameraListItem* AllCameras){
    //TODO free imoniker for every camera
    free(AllCameras);
    //TODO
    CoUninitialize();
}
